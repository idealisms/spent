...
class EditorInner extends React.Component<IEditorInnerProps, IEditorState> {
  constructor(props: IEditorInnerProps) {
    super(props);
  ...
  public componentDidUpdate(prevProps: IEditorInnerProps): void {
      ...
      <div className={classes.root}>
        ...
        <div className={classes.controls}>
          ...
          <DatePicker
            ...
            onChange={(d: moment.Moment | null) => this.handleChangeEndDate(d)}
            format="YYYY-MM-DD"
          />
  ...
  private handleChangeStartDate = (d: moment.Moment | null): void => {
    if (!d) {
      return;
    }
  ...
  private handleChangeEndDate = (d: moment.Moment | null): void => {
    if (!d) {
      return;
    }
  ...
  private handleAddTransaction = (
    ...
    this.setState({
      ...
      searchQuery: '',
      startDate:
        this.state.startDate > transactionDate
          ? transactionDate
          : this.state.startDate,
      endDate:
        this.state.endDate < transactionDate
          ? transactionDate
          : this.state.endDate,
    });
  };
    ...
    for (let t of this.props.transactions) {
      if (fromTransactions.has(t.id)) {
        ...
        for (let fromTransaction of fromTransactions.values()) {
          ...
          if (
            fromTransaction.transactions &&
            fromTransaction.transactions.length > 0
          ) {
...
function EditorWrapper(props: IEditorProps) {
  ...
  return <EditorInner {...props} classes={classes} />;
}
